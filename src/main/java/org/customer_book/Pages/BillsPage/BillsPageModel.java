...
public class BillsPageModel {
  ...
  private BooleanProperty showDeleteConfirmation = new SimpleBooleanProperty(false);

  // ------------- Status Update Properties -----------------//
  private StringProperty statusUpdateValue = new SimpleStringProperty("");
  private ObservableList<String> statusUpdateOptions = FXCollections.observableArrayList("UnPaid", "Paid",
      "Cancelled");
  ...
  private BooleanProperty showCompletedInvoice = new SimpleBooleanProperty(false);
  ...
  private BooleanProperty showGeneratedInvoicePopup = new SimpleBooleanProperty(false);
  private BooleanProperty showGenerateInvoicePane = new SimpleBooleanProperty(true);
  ...
  private BooleanProperty filterByDateCreated = new SimpleBooleanProperty(false);
  private BooleanProperty filterByDateCompleted = new SimpleBooleanProperty(false);
  ...
  /**
   * Constructor for the Bills Page Model - Load the customer names
   */
  public BillsPageModel() {
    ...
    jobToAdd.addListener((observable, oldValue, newValue) -> {
      if (newValue != null) {
        if (newValue.getCustomerName().equals(CustomerName.get()) || CustomerName.get().isEmpty()) {
  ...
  /**
   * generateRandomNumber - This function is used to generate a random number for
   * the invoice number
   * 
   * @return
   */
  ...
  /**
   * Load the bill cards - This function is used to load the bill cards for the
   * added jobs
   */
  private void loadBillCards() {
    ...
    for (JobDAO job : addedJobs) {
      ...
      try {
        ...
        ((BillJobCardController) cardLoader.getController()).setJobDAO(job, jobToRemove);
  ...
  private void updateJobDAOs(boolean clearExisting) {
    ...
    if (clearExisting) {
      completedJobDAOS = DatabaseConnection.jobCollection
          .getJobs(and(completedJobsFilter, ne("endDateTime", null)), 4, 0);
    } else {
      completedJobDAOS.addAll(DatabaseConnection.jobCollection
          .getJobs(and(completedJobsFilter, ne("endDateTime", null)), 2, completedJobDAOS.size()));
    }
  }

  /**
   * Load the completed jobs - This function is used to load the completed jobs
   * into the completedJobCards list
   */
  public void loadCompletedJobs() {
    ...
    for (JobDAO job : completedJobDAOS) {
      try {
        ...
        ((CompletedJobCardController) cardLoader.getController()).setJobDAO(job, jobToAdd, jobToRemove);
  ...
  public void saveInvoice() {
    ...
    mostRecentInvoiceId = DatabaseConnection.invoiceCollection.addInvoice(invoice);
    // Update the added Jobs with the invoice ID
    for (JobDAO job : addedJobs) {
      job.setInvoiceID(mostRecentInvoiceId);
      job.setHasInvoice(true);
  ...
  /**
   * Hide the generated invoice popup also reload the completed Jobs and the bill
   * cards
   */
  ...
  public void showGeneratedInvoicePopup() {
    ...
    throw new UnsupportedOperationException("Unimplemented method 'showGeneratedInvoicePopup'");
  }

  /**
   * Show the generate invoice pane and reload the completed jobs and the bill
   * cards
   */
  ...
  /**
   * Show the download location - This function is used to show the download
   * location for the invoices
   */
  public void showDownloadLocation() {
    DirectoryChooser directoryChooser = new DirectoryChooser();
    directoryChooser.setTitle("Select Download Location");
    directoryChooser
        .setInitialDirectory(downloadLocation.get().isEmpty() ? new File(System.getProperty("user.home"))
  ...
  public void showInvoiceFilter() {
    // TODO Auto-generated method stub
    throw new UnsupportedOperationException("Unimplemented method 'showInvoiceFilter'");
  }

  public void loadCompletedInvoiceDAOs(boolean clearExisting) {
    if (clearExisting) {
      completedInvoices = DatabaseConnection.invoiceCollection.getCompletedInvoices(completedJobsFilter, 4,
          0);
    } else {
      completedInvoices.addAll(DatabaseConnection.invoiceCollection.getCompletedInvoices(completedJobsFilter,
          2, completedInvoices.size()));
    }
  }

  /**
   * Load all of the completed invoices - This function is used to load all of the
   * completed invoices into the completedInvoiceCards list
   */
  public void loadCompletedInvoices() {
    ...
    for (InvoiceDAO invoice : completedInvoices) {
      try {
        FXMLLoader cardLoader = App.getLoader("BillsPage", "CompletedInvoiceCard");
        Parent card = cardLoader.load();
        ((InvoiceCardController) cardLoader.getController()).setInvoice(invoice, selectedInvoice);
  ...
  public void updateSelectedInvoice() {
    ...
    selectedInvoiceCustomerAddress.set(selectedInvoice.get().getCustomerAddress());
    selectedInvoiceCustomerPhoneNumber.set(selectedInvoice.get().getCustomerPhoneNumber());
    ...
    selectedInvoiceDeliveryTotal.set("$" + selectedInvoice.get().getDeliveryTotal());
    selectedInvoiceChargeTotal.set("$" + selectedInvoice.get().getChargeTotal());
  ...
  /**
   * Update the Job details list with a card for each bill for a job within the
   * selected invoice
   * 
  ...
  public void updateJobDetails(InvoiceDAO dao) {
    ...
    for (InvoiceEntry entry : dao.getBills()) {
      try {
        FXMLLoader cardLoader = App.getLoader("BillsPage", "SelectedInvoiceJobDetails");
        Parent card = cardLoader.load();
        ((InvoiceJobDetailsCardController) cardLoader.getController()).setJobDetails(entry, selectedInvoice);
  ...
  public void downloadSelectedInvoice() {
    try {
      ...
      ((DownloadInvoicController) downloadPopup.getController()).setProperties(selectedInvoice.get(),
          downloadLocation);
  ...
  /**
   * Save the invoice status - Update the invoice status of the selected invoice
   * 
   * @param value
   */
  public void saveInvoiceStatus(String value) {
    if (selectedInvoice.get() != null && !value.equals(selectedInvoice.get().getStatus())) {
  ...
  /**
   * Confirm the invoice delete remove the selected invoice from the database
   */
  ...
  public void applyJobsFilter() {
    ...
    completedJobsFilter = and(
        showGenerateInvoicePane.get()
            ? and(
                gt(filterByDateCreated.get() ? "created" : "endDateTime",
                    startDateProperty.getValue() != null ? startDateProperty.getValue().atStartOfDay()
                        : new Date(Long.MIN_VALUE)),
                lt(filterByDateCreated.get() ? "created" : "endDateTime",
                    endDateProperty.getValue() != null ? endDateProperty.getValue().atStartOfDay()
                        : new Date(Long.MAX_VALUE)))
            : (and(
                gt("generatedDateTime",
                    startDateProperty.getValue() != null ? startDateProperty.getValue().atStartOfDay()
                        : new Date(Long.MIN_VALUE)),
                lt("generatedDateTime",
                    endDateProperty.getValue() != null ? endDateProperty.getValue().atStartOfDay()
                        : new Date(Long.MAX_VALUE)))),
        customerNameJobsFilterProperty.get().equals("") ? ne("_id", null)
