...
public class HomePageConfig {
  ...
  public HomePageConfig() {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      File paneConfigFile = new File(
        DataFolder + "/PaneConfig" + (i + 1) + ".txt"
      );
  ...
  /**
   * load the PaneConfig from the file
   * - If the file does not exist create a new PaneConfig
   */
  private void loadPaneConfig(File paneConfigFile, int paneNumber) {
    try (
      ObjectInputStream ooi = new ObjectInputStream(
        new FileInputStream(paneConfigFile)
      )
    ) {
  ...
  public void savePaneConfig(PaneConfig paneConfig, int paneNumber) {
    ...
    try (
      ObjectOutputStream oos = new ObjectOutputStream(
        new FileOutputStream(paneConfigFile)
      )
    ) {
  ...
  /**
   * Load the HomePageConfig from the file
   *  - If the file does not exist create a new HomePageConfig
   * @return
   */
  private void loadHomePageConfig() {
    try (
      ObjectInputStream ooi = new ObjectInputStream(
        new FileInputStream(homeConfigFile)
      )
    ) {
  ...
  private void saveHomePageConfig(HomePageConfigPermenant homePageConfig) {
    try (
      ObjectOutputStream oos = new ObjectOutputStream(
        new FileOutputStream(homeConfigFile)
      )
    ) {
